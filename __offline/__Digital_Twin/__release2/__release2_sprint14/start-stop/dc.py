...
# def _load_values(table_name, asset_id, sort_field=None, start=None, stop=None, descending=True, limit=None):
#     """ fetch multiple rows from the database """
#     converted_start = None
#     converted_stop = None
#
...
#             start, stop)
#
#     extra_filters = {"Description": "Master Data Change"}  # SAP Equipment History
#
#     data_access = _get_data_access()
#
#     try:
...
def _parse_date(start, date):
    """ convert compressed iso time "20181009T160920Z" to
    uncompressed times 2018-10-09T16:09:20+00:00
    """
    try:
        start_time = dateutil.parser.parse(start)
        stop_time = dateutil.parser.parse(date)
        ...
        message = "Failed to find convert date ({}) or start ({}) time".format(
            start, date)
        logger.exception(message)
        raise BadRequest({"code": "parse stop and start failed",
                          "description": message}, status_code=400)
...
def get_ron_data_by_asset_id_and_time_and_doc(asset_id):  # pylint: disable=invalid-name
    """Get River of News data by asset id
    ...
    pprint(start)
    print("\n\n")
    # start = "20161009T150920Z"
    stop = data.get('stop')
    # stop = request.args.get('stop')
    print("\n\nstop: ")
    pprint(stop)
    print("\n\n")
    # stop = "20191009T160920Z"
    ...
    pprint(description)
    print("\n\n")
    # doc = ["Master Data Change", "SAP Equipment History"]
    # description = ["FSW Version"]
    # doc = ["SAP Equipment History", "Master Data Change", "FSW Version", "BoM as built set"]
    ...
    if date:
        ...
        pprint(start)
        print("\n\n")
        converted_start, converted_stop = _parse_date(
            start, date)
    ...
    if description:
        ...
        pprint(description)
        print("\n")
        extra_filters = {"Description": {"$in": description}}
    ...
    try:
        ...
        result_obj = data_access.fetch_many(
            BaseView.RON_DATA_TABLE, asset_id, DataAccess.IGNORE_FIELDS, start=converted_start,
            sort_field="Ts", stop=converted_stop, order_desc=True, extra_filters=extra_filters, limit=limit,
        )
        ...
        logger.exception("querying %s table failed", BaseView.RON_DATA_TABLE)
        raise
    finally:
        data_access.close()
