...
log = logging.getLogger(__name__)       ### want to know this desperatly ....???


class sap_equipment_history_Integrator(IntegratorCallbacks, ThingRunner):    ####  IntegratorCallbacks is might be for feed data and action requests stoaring, ?@ where does this feed data and action requests stoaring...????

    def __init__(self, config, agent_config):   ###  where does config, agent_config came from...?????

        super().__init__(config=agent_config)           ### reinitializing class 'ThingRunner' using super() here
...
#??     # Whilst the integrator core requires particular configuration, top-level sections could be defined to provide   ###?? which top level section???
        ...
        self.__integrator = Integrator(config['integrator'], self.client, self) ### 'self.__integrator' object created
    ...
    def on_shutdown(self, exc_info):         ### it commanded to shutdown then asking integrator to stop.
    ...
    def on_asset_created(self, asset_id):           ### this will be called at every control_requests and feeddata_request.
        log.debug('Asset created: %s', asset_id)    ### 
        self.__assets.add(asset_id)

    # for IntegratorCallbacks
    def on_asset_deleted(self, asset_id):           ### this will be called at every control_requests and feeddata_request.
        log.debug('Asset deleted: %s', asset_id)
        self.__assets.discard(asset_id)

    # for IntegratorCallbacks                         ### this will be called at every control_requests and feeddata_request.
    def on_t2_request(self, request):                  ### T2 request comes from app to sys
        pass
####
                ...
                if self._has_asset_data_changed_for(asset_id, data):    #### calling it to '_has_asset_data_changed_for ()'  @down
                    ...
                    if event_time is not None:
                        try:
                            ...
                            log.error(ex)
                    event = SapMasterDataSet(asset_id, data=master_data, time=event_time)       #(need to give history time)## SapMasterDataSet @(kafka) extracting event history 
