...
class TalendFirmwareIntegrator(IntegratorCallbacks, ThingRunner):
    ...
    def _get_data_for_asset(self, asset_id):
        ...
        if self.__config['config']['use_mock_data'] == 1:
            ...
            endpoint = NestedConfig.get(self.__config, 'config.sap.endpoint', required=True, check=non_empty_str)
            ...
            timeout = int(NestedConfig.get (self.__config, 'config.sap.timeout', required=False, default=10, check=non_negative_int)
