...
# 8.2_actuate_control_ask.py 12/02/2016 Iotic Labs.
# Creates a new Thing (or connects an existing one).
# Prints information about that Thing (Thing object, lid, guid).
# Attaches to a remote Control using hard-coded Point guid.
# Actuates remote Control using ask.
...
def attach_to_control(my_thing):
    ...
    # attach to Control
    my_controller = my_thing.attach(remote_control_gpid)
    print("Remote Control subscription object:", my_controller)

    return my_controller


def actuate_ask(my_controller):
    print("Actuating remote Control - ask")

    # Actuate remote Control (ask)
    # Note:
    #   There is no notification of success when 'asking'
    print("Asking remote Control to act")
    data = {'something_useful': 'useful value'}
    # send data to the Control (ask)
    my_controller.ask(data)
...
def main():
    with IOT.Client('Control_Actuate.ini') as client:
        ...
        my_controller = attach_to_control(my_thing)

        while True:
            try:
                print("Main running, press ctrl+c to quit.")
                actuate_ask(my_controller)
                time.sleep(10)
