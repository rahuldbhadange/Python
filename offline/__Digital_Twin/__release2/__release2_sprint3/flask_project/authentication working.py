...
conn = http.client.HTTPConnection('http://127.0.0.1:5000/sapwarrantyrecall', 80, timeout=10)
app = Flask(__name__)  # Construct an instance of Flask class for our webapp
app.config["DEBUG"] = True  # Enable reloader and debugger


sapwarrantyrecall_mock_data = { 'd' : {"results":
                                    [
    {
     "__metadata": {
...
@app.route('/sapwarrantyrecall/', methods=['GET'])
def sapwarrantyrecall_auth():
    username = request.args['usr']
    password = request.args['pwd']
    if not check_auth(username, password):
        """Sends a 401 response that enables basic auth"""
        return 'Could not verify your access level for that URL. You have to login with proper credentials', 401
...
@app.route('/sapwarrantyrecall&&<username>&<password>', methods=['GET'])  # Variable with type filter. Accept only int
